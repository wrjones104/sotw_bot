...
async def generate_seed(flags, seed_desc):
    url = constants.url
    payload = json.dumps(
        {"key": constants.new_api_key, "flags": flags, "description": seed_desc}
    )
    headers = {"Content-Type": "application/json"}
...
async def create_new_sotw(ctx, name, submitter, flags, description):
    ...
    with open("sotw_db.json") as x:
        sotw_db = json.load(x)
    try:
        participants = await sotw_channel.fetch_message(
            sotw_db[str(len(sotw_db))]["participants_msg_id"]
        )
        rankings = await leaderboard_channel.fetch_message(
            sotw_db[str(len(sotw_db))]["rankings_msg_id"]
        )
    ...
    sotw_db[len(sotw_db) + 1] = {
        ...
        "header_msg_id": str(sotw_header.id),
        "leaderboard_header_id": str(leader_header.id),
        "spoiler_splitter_id": str(spoiler_splitter.id),
        "rankings_msg_id": str(rankings.id),
        "participants_msg_id": str(participants.id),
        "runners": {},
    }
    ...
    cur = con.cursor()
    cur.execute(
        "UPDATE presets SET flags = (?), description = (?) WHERE preset_name = (?)",
        (
            flags,
            f"Practice for this week's SotW: **{name}** by {submitter}\n```{description}```",
            "SotW",
        ),
    )
    con.commit()
...
async def auto_create_new_sotw(ctx):
    ...
    sotw_channel = get(
        ctx.get_all_channels(), guild__id=constants.guild, name="seed-of-the-week"
    )
    leaderboard_channel = get(
        ctx.get_all_channels(), guild__id=constants.guild, name="sotw-leaderboards"
    )
    spoiler_channel = get(
        ctx.get_all_channels(), guild__id=constants.guild, name="sotw-spoilers"
    )
    general_channel = get(
        ctx.get_all_channels(), guild__id=constants.guild, name="ff6wc-general-chat"
    )
    ...
    if not settings:
        return print(
            f"{datetime.datetime.now()}: No settings options found - shutting down auto-roll workflow"
        )
    elif settings["auto-mode"] == "False":
        return print(
            f"{datetime.datetime.now()}: Auto-mode set to FALSE - shutting down auto-roll workflow"
        )
    error_check = True
    badflags = [""]
    while error_check:
        ...
        try:
            ...
            print(
                f"{datetime.datetime.now()}: There was a flag error with this submission: {name} from {submitter}"
            )
    ...
    await move_tabs(
        this_week, create_date, submitter, name, description, flags, seed_link, del_row
    )
    with open("sotw_db.json", "w") as updatefile:
        updatefile.write(json.dumps(sotw_db))
...
async def enter_time(ctx, time):
    ...
    if str(ctx.user.name) in sotw_db[str(len(sotw_db))]["runners"]:
        ...
        updated_participants_msg = (
            f"{len(sotw_db[str(len(sotw_db))]['runners'].values())} participants"
        )
...
async def refresh(ctx):
    ...
    sotw_header = await sotw_channel.fetch_message(
        sotw_db[str(len(sotw_db))]["header_msg_id"]
    )
    leader_header = await leaderboard_channel.fetch_message(
        sotw_db[str(len(sotw_db))]["leaderboard_header_id"]
    )
    spoiler_splitter = await spoiler_channel.fetch_message(
        sotw_db[str(len(sotw_db))]["spoiler_splitter_id"]
    )
    updated_rankings_msg = ""
    runner_list = []
    count = 0
...
async def new_submission(ctx):
    ...
    try:
        ...
        await write_new_submission(
            ctx, modal.sotwname, modal.sotwflags, modal.sotwdesc, link["url"]
        )
...
async def new_reserve_choice(ctx):
    ...
    try:
        ...
        return await ctx.user.send("Your reserve submission has been received!")
    except KeyError:
        return await ctx.user.send(
            "There seems to be something wrong with your flags - "
            "double-check them and try again!"
...
async def write_new_reserve(ctx, name, flags, desc):
    ...
    settings[len(settings) + 1] = {
        "name": str(name),
        "submitter": str(ctx.user.name),
        "flags": str(flags),
...
async def get_possible_seeds(badflags):
    ...
    try:
        ...
        print(
            f"{datetime.datetime.now()}: {len(random_select)} possible flagsets found!"
        )
...
async def move_tabs(
    ctx, create_date, submitter, name, description, flags, seed_link, del_row
):
    gc = pygsheets.authorize(service_file="functions/sotw-bot-eda350e55a58.json")
    sh = gc.open(constants.sheetname)
    wks = sh[0]
    ...
    wks2.insert_rows(
        lastrow,
        number=1,
        values=[create_date, submitter, name, description, flags, seed_link],
    )
